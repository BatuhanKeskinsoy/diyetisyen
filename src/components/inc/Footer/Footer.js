        ...
        <div className="flex gap-x-6 lg:flex-row flex-col lg:gap-y-0 gap-y-6">
          <div className='flex-1 flex flex-col gap-y-6'>
            <Image
              ...
              height={65}
              priority />
            <p className='line-clamp-3'>Sağlıklı bir yaşam için doğru beslenme uzmanıyla çalışın. Beslenme hedeflerinizi gerçekleştirmek için profesyonel desteğe sahip olun.</p>

            <ul className='flex gap-4 flex-wrap'>
              {
                SocialsData.map((data, key) => (
                  <FooterSocials socialUrl={data.socialUrl} socialName={data.socialName} iconName={data.iconName} key={key} />
                ))
              }
            </ul>
          ...
          <div className='flex-1'>
            <span className='font-gemunu text-2xl tracking-wide'>Hızlı Linkler</span>
          ...
          <div className='flex-1'>
            <span className='font-gemunu text-2xl tracking-wide'>Hizmetlerimiz</span>
            <hr className='my-2 border-gray-300' />
            <ul>
              {
                servicesData.slice(0, 6).map((serviceItem, key) => (
                  <li key={key}>
                    <Link href={`/hizmetlerimiz/${serviceItem.url}`} title={serviceItem.title} className='w-fit flex items-center gap-x-1 py-1 font-gemunu tracking-wide text-lg hover:text-site hover:pl-1 transition-all'>
                      {serviceItem.title}
                    </Link>
                  </li>
                ))
          ...
          <div className='flex-1'>
            <span className='font-gemunu text-2xl tracking-wide'>Hesaplamalar</span>
            <hr className='my-2 border-gray-300' />
            <ul>
              {
                calculationsData.slice(0, 6).map((calculateItem, key) => (
                  <li key={key}>
                    <Link href={`/hesaplamalar/${calculateItem.url}`} title={calculateItem.title} className='w-fit flex items-center gap-x-1 py-1 font-gemunu tracking-wide text-lg hover:text-site hover:pl-1 transition-all'>
                      {calculateItem.title}
                    </Link>
                  </li>
                ))
            ...
            <ul className="flex flex-col gap-y-3">
              {
                filteredBlogs.length > 0 ? (
                  ...
                  <div className='font-gemunu tracking-wider text-left w-full'>Blog Bulunamadı...</div>
              }
            </ul>
          </div>
        </div>
