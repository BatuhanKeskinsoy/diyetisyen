...
import { Meta404 } from '@/meta/meta404'
            ...
            <section className='container mx-auto px-4 flex flex-col justify-between items-center h-[calc(100vh-136px)] mt-8 lg:mt-0'>
                <div className="flex lg:flex-row flex-col items-center h-full justify-center gap-6">
                    <div className="flex flex-col gap-y-10 lg:order-1 order-2">
                        <Link href={'/'} title={process.env.NEXT_PUBLIC_SITE_NAME} className='lg:order-1 order-4'>
                            <Image
                        ...
                        <h1 className='font-gemunu tracking-wide text-4xl lg:text-6xl text-center'>Sayfa Bulunamadı</h1>
                        <p className='text-base text-gray-600 text-center'>Üzgünüz, aradığınızı bulamadık. Bu sayfa, silinmiş veya değiştirilmiş olabilir.</p>
                        <Link href={'/'}
                            title={process.env.NEXT_PUBLIC_SITE_NAME}
